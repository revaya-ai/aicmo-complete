...
- **B3.10 Notion mirror (DONE)**: mirror the pipeline to a Notion board so the client approves from their phone instead of the Flask page. Built as `engine/dashboard/notion_mirror.py`: offline it writes a JSON board to `outputs/notion-mirror.json` (mode "stub"); with `NOTION_TOKEN` set it pushes the same cards to a real Notion database. `mirror()` mirrors the whole pipeline, `mirror_gate()` mirrors only `qc_review` items for "approve from your phone". The human gate (`gate.py`) exposes `mirror_to_notion()`. **Done.**
...
- **B4.2 ad-creative render (Studio contributes) (DONE)**: `render.py` variant at ad dimensions. Built as `render_ad(post_id)` in `engine/studio/render.py`, producing a 1080x1080 placeholder PNG (stdlib) or a real Playwright render when `AICMO_RENDER=playwright`. Wired into `ads_push.run` so the creative renders before the campaign goes live. **Done.**
...
- **B4.5 ads_push.py**: reads `ad_approved`, **writes `ad_live`**. Real Meta/LinkedIn Ads call if tokens set, otherwise stub returning a fake campaign id into `ad_status`. **Done when** an approved ad reaches `ad_live` in stub mode. (The real API sits behind a token env check; the stub is the floor and runs offline. Ads auth must never block the content loop.)
...
Polish bricks (B2.4 brand-test depth, B3.5 publish-linkedin, real Zernio, real Ads API) are upgrades you layer on after the loop is green. B3.10 Notion is now built as a stub (offline JSON board, real Notion behind `NOTION_TOKEN`).

---

## Layer 6: Full architecture coverage (every component represented)

These bricks complete the reference architecture so nothing is silently missing. Each is real-but-stubbed: stdlib only on the default path, no keys, no network. Real services sit behind env checks.

- **B6.1 Intelligence layer (DONE)**: `engine/intelligence/intelligence.py` produces deterministic candidate seed ideas grounded in the client's `strategy.md` pillars. Skill `intelligence`, command `/ai-cmo-intel`, test `tests/test_intelligence.py`. Real DataForSEO/GSC/Apify behind env checks.

- **B6.2 Feedback loop (DONE)**: `engine/feedback.py` reads `analyzed` posts, ranks winners by engagement rate, and appends a learnings note to `client-data/<client>/learnings.md`. Skill `feedback-loop`, test `tests/test_feedback.py`.

- **B6.3 Dashboard + reporting (DONE)**: `engine/dashboard/metrics.py` aggregates the pipeline (counts by status, top posts, stub run cost); `engine/dashboard/report.py` writes a weekly markdown brief to `outputs/reports/`; `engine/dashboard/notion_mirror.py` is B3.10. Command `/ai-cmo-report`, tests `tests/test_dashboard.py`.

- **B6.4 Notion human-gate mirror (DONE)**: `gate.py` exposes `mirror_to_notion()` which mirrors only `qc_review` items to the notion_mirror stub so a human can "approve from your phone". Test `tests/test_gate_notion_mirror.py`. Notion never required to run.

- **B6.5 Pattern libraries (DONE)**: `hook-library` and `story-structures` skills, referenced by `content-os` at the Hook and Story steps.

- **B6.6 Onboarding + multi-repo model (DONE)**: command `/ai-cmo-onboard` (a new client becomes the six markdown layers plus brand spec), doc `docs/architecture/multi-repo-model.md` (AIOS to AI CMO Core to per-client box), and `engine/leak_guard.py` (a client box contains no other client's name) with test `tests/test_leak_guard.py`.

- **B6.7 Persona registry (DONE)**: `docs/architecture/agents.md` maps every marketing-department persona (Analyst, Strategist, Copywriter, Creative, Systems/QA, Publisher, Ads, the human gate, Feedback) to the skills, commands, and engine modules that implement it.
