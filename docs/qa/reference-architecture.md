...
| Gemini / Nano Banana | Image model option for Studio render. | `GEMINI_API_KEY`, selected with `AICMO_RENDER=gemini` | `GeminiImageClient` (`engine/integrations/gemini.py`) raises `GeminiNotConfigured` before any network call when the key is absent; `engine/studio/render.py` `_render_with_gemini` returns False on missing key or any error and falls back to the offline render (PIL default, then stdlib placeholder PNG). Real path only taken when `AICMO_RENDER=gemini` and the key is set. | WIRED |
...
| GSC (Google Search Console) | Intelligence layer search-console signals. | `GSC_ACCESS_TOKEN`, `GSC_SITE_URL` (both required) | `GSCClient` (`engine/integrations/gsc.py`) raises `GSCNotConfigured` before any network call unless both are set; the Intelligence layer (`sweep_with_meta`) adds `live:gsc` seeds when configured, else falls back to the deterministic stub. Consumes a pre-obtained OAuth2 bearer token, stdlib only, no google-auth dependency. | WIRED |
| Apify | Intelligence layer competitor scrapes. | `APIFY_TOKEN` | `ApifyClient` (`engine/integrations/apify.py`) raises `ApifyNotConfigured` before any network call when the token is absent; the Intelligence layer adds `live:apify` seeds when configured, else falls back to the stub. Token sent in the Authorization header, never the URL. | WIRED |
...
| Backblaze B2 | Nightly encrypted backups and the client no-AI-image asset store. | `BACKBLAZE_KEY_ID`, `BACKBLAZE_APPLICATION_KEY`, `BACKBLAZE_BUCKET` | `BackblazeClient` (`engine/integrations/backblaze.py`) raises `BackblazeNotConfigured` before any network call unless all three are set; `engine/mission/backup.py` falls back to a local offline manifest in `outputs/backups/` when unconfigured. Stdlib only (3-step B2 native API). | WIRED |
